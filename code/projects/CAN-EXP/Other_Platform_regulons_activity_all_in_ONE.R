...
d6<- data.frame(d5)
row.names(d5)<- paste("G", row.names(d5), sep='')
...
write.csv(d5, "feed2python.csv", row.names=T, quote=F)
...
rm(d3,d4,d5,d6,matrix1,overlapped_tfs, corrMat, cor_tfs,network, netGet,chipbase_links,cor_tfs_spearman,chipseq_tg,GRNboost2_tg,
